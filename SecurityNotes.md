...
Enforcement controls what the identity can do; data posture controls what the model provider sees.
RBAC solves only the first, and the two degrade independently:
...
As content sensitivity rises the posture degrades in a known order: retention settings stop being
acceptable first, then plaintext secret storage, then the shared service principal itself.

## What this looks like at enterprise scale

The three decisions carry over (dedicated identity, minimal scope, service-side enforcement), but
nearly every component gets swapped for a managed equivalent:
